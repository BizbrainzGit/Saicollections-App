...
if (isset($_POST['update_web_category']) && $_POST['update_web_category'] == 1) {
    ...
    if ($permissions['categories']['update'] == 1) {
        ...
        $extension = pathinfo($db->escapeString($fn->xss_clean($_FILES["c_image"]["name"])['extension']));
        $result = $fn->validate_image($fn->xss_clean_array($_FILES["c_image"]));
        if ($result) {
            echo '<p class="alert alert-danger">Image type must jpg, jpeg, gif, or png!</p>';
            return false;
            exit();
...
if (isset($_POST['seller_id']) && !empty($_POST['seller_id']) && isset($_POST['transfer_fund_seller']) && !empty($_POST['transfer_fund_seller'])) {
    ...
    if ($fn->add_wallet_transaction($order_id = "", 0, $id, $type, $amount, $message, 'seller_wallet_transactions')) {
...
if (isset($_POST['update_return_request']) && $_POST['update_return_request'] == 1) {
    ...
    if ($return_status == 1) {
        ...
        if ($returned == $total_order) {
            echo $sql = "update orders set delivery_charge=0,tax_amount=0,tax_percentage=0,final_total=0,total=0 where id=" . $order_id;
            $db->sql($sql);
        }
    }
            ...
            if ($count1 != 0) {
                ...
                if ($total > 3) {
                    ...
                    $data = array(
                        'product_id' => $res[0],
                        'type' => $emapData[11],
                        'measurement' => $emapData[12],
                        'measurement_unit_id' => $emapData[13],
                        'price' => $emapData[14],
                        'discounted_price' => $emapData[15],
                        'serve_for' => $emapData[16],
                        'stock' => $emapData[17],
                        'stock_unit_id' => $emapData[18]
                    );
                    $db->insert('product_variant', $data);
                    $res1 = $db->getResult();
                }
...
if (isset($_POST['add_seller']) && $_POST['add_seller'] == 1) {
    ...
    $sql = "INSERT INTO `seller`(`name`, `store_name`, `email`, `mobile`, `password`, `store_url`, `logo`, `store_description`, `street`, `pincode_id`,`city_id`, `state`, `account_number`, `bank_ifsc_code`, `account_name`, `bank_name`, `commission`,`status`,`categories`,`require_products_approval`,`national_identity_card`,`address_proof`,`pan_number`,`tax_name`,`tax_number`,`customer_privacy`,`latitude`,`longitude`) VALUES ('$name','$store_name','$email', '$mobile', '$password','$store_url' ,'$filename', '$store_description', '$street',$pincode_id,$city_id,'$state','$account_number','$bank_ifsc_code','$account_name','$bank_name','$commission','$status','$cat_ids','$require_products_approval','$national_id_card','$address_proof','$pan_number','$tax_name','$tax_number','$customer_privacy','$latitude','$longitude')";
    if ($db->sql($sql)) {
